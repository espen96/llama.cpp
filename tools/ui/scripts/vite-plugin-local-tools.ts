...
function proxyToServer(
	...
	return new Promise((resolve) => {
		const url = new URL(urlPath, SERVER_ORIGIN);
		const options = {
	...
	return {
		...
		configureServer(server) {
			server.middlewares.use(async (req, res, next) => {
				...
				if (req.method === 'GET') {
					const localTools = [EXECUTE_JAVASCRIPT_TOOL_INFO];

					const serverResult = await proxyToServer('/tools', 'GET');
					let remoteTools: any[] = [];
					if (serverResult && serverResult.status === 200 && Array.isArray(serverResult.data)) {
						remoteTools = serverResult.data;
					}
				...
				if (req.method === 'POST') {
					...
					req.on('end', async () => {
						...
						const serverResult = await proxyToServer('/tools', 'POST', bodyBuffer.toString());
						if (serverResult) {
							res.statusCode = serverResult.status;
							res.setHeader('Content-Type', 'application/json');
							res.end(JSON.stringify(serverResult.data));
						} else {
							res.statusCode = 502;
							res.setHeader('Content-Type', 'application/json');
