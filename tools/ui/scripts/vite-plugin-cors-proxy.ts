				...
				if (urlObj.pathname === '/cors-proxy') {
					...
					try {
						...
						req.on('end', () => {
							...
							const makeRequest = (currentUrl: URL, redirectCount = 0) => {
								if (redirectCount > 5) {
									res.statusCode = 502;
									res.setHeader('Content-Type', 'application/json');
									res.end(JSON.stringify({ error: 'Too many redirects in CORS proxy' }));
									return;
								}

								const requestModule = currentUrl.protocol === 'https:' ? https : http;

								// Ensure Host header matches the current target host
									...
									(clientRes) => {
										...
										if (isRedirect && location) {
											try {
												...
												console.log(`[CORS Proxy] Redirect ${statusCode} to: ${nextUrl}`);
												makeRequest(nextUrl, redirectCount + 1);
												return;
											} catch (e: any) {
												console.error(`[CORS Proxy] Failed to parse redirect location: ${location}`, e);
											}
										}
